...
const Player = () => {
  ...
  return (
    <div className="sticky bottom-0 w-full sm:px-12 px-8 flex justify-between rounded-lg h-20 bg-gradient-to-r from-accent to-black">
      <div className="basis-1/4">
        <div className="flex gap-3">
          <img
            src={currentSong?.imageUrl}
            alt="song"
            className={`m-3 h-14 w-14 rounded-full border-4 ${
              isPlaying ? "animate-spin duration-1000" : ""
            }`}
          />

          <div className="text-white w-25 mt-3">
            <h6 className="truncate ">{currentSong?.trackName}</h6>
            <h6 className="truncate">{currentSong?.artistName}</h6>
          </div>
        </div>{" "}
      </div>
      ...
      <div className="hidden md:flex basis-1/4 items-center justify-en">
        {volume <= 1 && volume > 0.5 ? (
